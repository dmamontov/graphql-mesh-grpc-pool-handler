...
export default class GrpcHandler implements MeshHandler {
    private readonly config: GrpcPoolHandler;
    private readonly baseDir: string;
    private readonly schemaWithAnnotationsProxy: StoreProxy<GraphQLSchema>;
    ...
    constructor({
        ...
        pubsub,
        importFn,
    }: MeshHandlerOptions<GrpcPoolHandler>) {
        this.logger = logger;
        this.config = config;
    ...
    async processReflection(creds: ChannelCredentials): Promise<Array<Promise<protobufjs.Root>>> {
        this.logger.debug(`Using the reflection`);
        const reflectionEndpoint = stringInterpolator.parse(this.config.endpoint, {
            env: process.env,
        });
        ...
        const services: Array<string | void> = await reflectionClient.listServices();
    ...
    async processDescriptorFile() {
        ...
        if (options.includeDirs) {
            if (!Array.isArray(options.includeDirs)) {
                throw new TypeError('The includeDirs option must be an array');
            }
    ...
    async getDescriptorSets(creds: ChannelCredentials) {
        const rootPromises: Array<Promise<protobufjs.Root>> = [];
    ...
    walkToFindTypePath(
        ...
        while (
            !lodashHas(rootJson.nested, currentWalkingPath.concat(baseTypePath).join('.nested.'))
        ) {
    ...
    private readonly grpcObjectByservicePoolClientByObjPath = new WeakMap<
        ReturnType<typeof loadPackageDefinition>,
        Map<string, genericPool.Pool<any>>
    >();
    ...
    getServiceClient({
        ...
        if (typeof ServiceClient !== 'function') {
            throw new TypeError(`Object at path ${objPath} is not a Service constructor`);
        }
    ...
    getServiceClientPool({
        ...
        let servicePoolClientByObjPath =
            this.grpcObjectByservicePoolClientByObjPath.get(grpcObject);
        if (!servicePoolClientByObjPath) {
            servicePoolClientByObjPath = new Map<string, genericPool.Pool<any>>();
            this.grpcObjectByservicePoolClientByObjPath.set(grpcObject, servicePoolClientByObjPath);
        }

        let pool: genericPool.Pool<any> | undefined = servicePoolClientByObjPath.get(objPath);
        if (!pool) {
            const factory: genericPool.Factory<any> = {
                create: async () => this.getServiceClient({ grpcObject, objPath, creds }),
                destroy: async (client: ServiceClient) => {
                    client.close();
                },
            };

            pool = genericPool.createPool(factory, this.config.pool);
            ...
            servicePoolClientByObjPath.set(objPath, pool);
        }
    ...
    getFieldResolver({
        ...
        return async (root, args, context) => {
            return pool.acquire().then(async (client: ServiceClient) => {
    ...
    processDirectives({ schema, creds }: { schema: GraphQLSchema; creds: ChannelCredentials }) {
        ...
        for (const { name, rootJson, loadOptions } of rootJsonAnnotations) {
            ...
            grpcObjectByRootJsonName.set(
                name,
                this.getGrpcObject({ rootJson, loadOptions, rootLogger }),
            );
        }
                        ...
                        [fieldName]: {
                            type: () => {
                                ...
                                if (keyType) {
                                    ...
                                    fieldInputTypeName = getTypeName(
                                        this.schemaComposer,
                                        fieldTypePath,
                                        true,
                                    );
                ...
                const fieldConfigArgs = {
                    input: () => {
                        ...
                        if (baseRequestTypePath) {
                            ...
                            const requestTypeName = getTypeName(
                                this.schemaComposer,
                                requestTypePath,
                                true,
                            );
    ...
    private readonly schemaComposer = new SchemaComposer();
    ...
    async getMeshSource({ fetchFn }: GetMeshSourcePayload) {
        ...
        this.config.requestTimeout = this.config.requestTimeout || 200_000;
